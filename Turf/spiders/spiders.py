...
class ReunionSpider(Spider):
    ...
    start_urls = ["http://aspiturf.com/PTcoursejourdet.php?id=" + str(687312 + i) for i in range(255000)]
        
    def parse(self, response):
	selector = Selector(response)
	questions = selector.xpath("""//div[@id="grandtab"]/div[@id="tableau"]/table[@id="grdatatablesauto"]/tbody/tr""")

        self.logger.info("Extract %s", str(questions.extract()))
	item = []
	for sel in questions:
	    item = ReunionItem()
	    ...
	    item['Infojojo'] = sel.xpath("td[1]/a[2]/@infojojo").extract()
	    item['Title_Entraineur'] = sel.xpath("td[1]/a[2]/@title").extract()
            item['Entraineur'] = sel.xpath("td[1]/a[2]/text()").extract()
	    ...
	    item['MusiquePT'] = sel.xpath("td[12]/text()").extract()
            item['MusiqueCal'] = sel.xpath("td[13]/@title").extract()
	    item['MusiqueJoc'] = sel.xpath("td[14]/@title").extract()
	    item['MusiqueEntr'] = sel.xpath("td[15]/@title").extract()
